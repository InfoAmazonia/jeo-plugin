...
/* harmony import */ var lodash_template__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! lodash.template */ "./node_modules/lodash.template/index.js");
...
var JeoMap = /*#__PURE__*/function () {
  function JeoMap(element) {
    var _this = this;

    _babel_runtime_helpers_classCallCheck__WEBPACK_IMPORTED_MODULE_0___default()(this, JeoMap);
    ...
    this.moreInfoTemplate = lodash_template__WEBPACK_IMPORTED_MODULE_2___default()(window.jeoMapVars.templates.moreInfo);
    this.popupTemplate = lodash_template__WEBPACK_IMPORTED_MODULE_2___default()("<article class=\"popup popup-wmt\">".concat(window.jeoMapVars.templates.postPopup, "</article>"));
    this.initMap().then(function () {
      if (_this.getArg('layers') && _this.getArg('layers').length > 0) {
        map.setZoom(_this.getArg('initial_zoom'));
        map.setCenter([_this.getArg('center_lon'), _this.getArg('center_lat')]);
        map.addControl(new mapboxgl.NavigationControl({
          showCompass: false
        }), 'top-left');

        if (_this.getArg('disable_scroll_zoom')) {
          map.scrollZoom.disable();
        }

        if (_this.getArg('disable_drag_pan')) {
          map.dragPan.disable();
          map.touchZoomRotate.disable();
        }

        if (_this.getArg('disable_drag_rotate')) {
          map.dragRotate.disable();
        }

        if (_this.getArg('enable_fullscreen')) {
          map.addControl(new mapboxgl.FullscreenControl(), 'top-left');
        }

        if (_this.getArg('max_bounds_ne') && _this.getArg('max_bounds_sw') && _this.getArg('max_bounds_ne').length === 2 && _this.getArg('max_bounds_sw').length === 2) {
          map.setMaxBounds([_this.getArg('max_bounds_sw'), _this.getArg('max_bounds_ne')]);
        }

        if (_this.getArg('min_zoom')) {
          map.setMinZoom(_this.getArg('min_zoom'));
        }

        if (_this.getArg('max_zoom')) {
          map.setMaxZoom(_this.getArg('max_zoom'));
        }
      }
    }).then(function () {
      // Show a message when a map doesn't have layers
      if (_this.getArg('layers') && _this.getArg('layers').length === 0) {
        _this.addMapWithoutLayersMessage();
      } else {
        var amountLayers = 0;
        ...
        _this.getLayers().then(function (layers) {
          ...
          if (_this.getArg('layers') && _this.getArg('layers').length > 0 && amountLayers > 0) {
            var baseLayer = layers[0];
            baseLayer.addStyle(map);
            var customAttribution = [];
            map.on('load', function () {
              layers.forEach(function (layer, i) {
                if (layer.attribution) {
                  var attributionLink = layer.attribution;
                  var attributionName = layer.attribution_name;
                  ...
                  if (layer.attribution && !layer.attribution.includes('http')) {
                    if (layer.attribution.match(regex)) {
                      attributionLink = "https://".concat(layer.attribution);
                    }
                  }

                  var attributionLabel = attributionName.replace(/\s/g, '').length ? attributionName : attributionLink;
                  customAttribution.push("<a href=\"".concat(attributionLink, "\">").concat(attributionLabel, "</a>"));
                }
            ...
            _this.addLayersControl(amountLayers);

            map.addControl(new mapboxgl.AttributionControl({
              customAttribution: customAttribution
            }), 'bottom-left');

            _this.addMoreButtonAndLegends();
          }

          _this.getRelatedPosts(); // Show a message when a map doesn't have layers
  ...
  _babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_1___default()(JeoMap, [{
    key: "initMap",
    ...
    key: "addMapWithoutLayersMessage",
    value: function addMapWithoutLayersMessage() {
    ...
    key: "addMoreButtonAndLegends",
    value: function addMoreButtonAndLegends() {
      var _this3 = this;

      var container = document.createElement('div');
      container.classList.add('legend-container');
      var hideableContent = document.createElement('div');
      hideableContent.classList.add('hideable-content');
      var appearingLegends = 0;
      ...
      if (this.legends.length > 0 && appearingLegends > 0) {
        ...
        legendsHideIcon.addEventListener('click', function () {
        ...
        var legendsWrapper = document.createElement('div');
      ...
      if (this.map_post_object) {
        ...
        this.layers.forEach(function (layer) {
          innerHTML += "<h3>".concat(layer.attributes.layer_name, "</h1>");
          var regex = new RegExp(/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)?/gi);
          var attributionLink = layer.attributes.attribution;
          var attributionName = layer.attributes.attribution_name;
          var sourceLink = layer.source_url;

          if (layer.attributes.attribution && !layer.attributes.attribution.includes('http')) {
            if (layer.attributes.attribution.match(regex)) {
              attributionLink = "https://".concat(layer.attributes.attribution);
            }
          }

          if (layer.source_url && !layer.source_url.includes('http')) {
            if (layer.source_url.match(regex)) {
              sourceLink = "https://".concat(layer.source_url);
            }
          }

          if (attributionLink) {
            innerHTML += "<p>Attribution: <a href=\"".concat(attributionLink, "\">").concat(attributionName, "</a></p>");
          }

          if (sourceLink) {
            innerHTML += "<a\n\t\t\t\t\t\t\t\t\tstyle=\"font-family: 'Helvetica Neue', Arial, Helvetica, sans-serif;\n\t\t\t\t\t\t\t\t\tbackground: #fff;\n\t\t\t\t\t\t\t\t\tborder: 1px solid rgba(0,0,0,0.4);\n\t\t\t\t\t\t\t\t\tcolor: #404040;\n\t\t\t\t\t\t\t\t\tmargin-top: 8px;\n\t\t\t\t\t\t\t\t\tpadding: 4px 10px;\n\t\t\t\t\t\t\t\t\ttext-decoration: none;\n\t\t\t\t\t\t\t\t\tborder-bottom: 1px solid rgba(0,0,0,0.25);\n\t\t\t\t\t\t\t\t\ttext-align: center;\n\t\t\t\t\t\t\t\t\tcursor: pointer;\n\t\t\t\t\t\t\t\t\tdisplay: inline-block;\n\t\t\t\t\t\t\t\t\tfont-size: 16px;\n\t\t\t\t\t\t\t\t\tfont-weight: bold;\n\t\t\t\t\t\t\t\t\ttransition: all .2 ease-in-out;\"\n\t\t\t\t\t\t\t\t\thref=\"".concat(sourceLink, "\" class=\"download-source\">Download from source\n\t\t\t\t\t\t\t\t  </a>");
          }
        });
      ...
      this.element.appendChild(container); // hide legends from hidden layers
      ...
      this.layers.forEach(function (l, i) {
        ...
        if (l.attributes.visible !== true) {
          jQuery(_this3.element).find('.legend-for-' + l.layer_id).hide();
        }
      });
    }
  }, {
    ...
    value: function getLayers() {
      ...
      return new Promise(function (resolve, reject) {
        var layersDefinitions = _this4.getArg('layers');

        _this4.layersDefinitions = layersDefinitions;

        if (layersDefinitions) {
          var layersIds = layersDefinitions.map(function (el) {
            return el.id;
          });
          var urlRoutes = window.location.pathname.split('/');
          var lang = urlRoutes[1];
          jQuery.get(jeoMapVars.jsonUrl + 'map-layer', {
            include: layersIds,
            orderby: 'include',
            lang: lang ? lang : ''
          }, function (data) {
            ...
            var returnLegends = [];
            var ordered = [];
            layersIds.forEach(function (el, index) {
              ordered[index] = data.find(function (l) {
                return l.id == el;
              });
            });
            ordered.forEach(function (layerObject, i) {
              if (layerObject) {
                returnLayers.push(new window.JeoLayer(layerObject.meta.type, {
                ...
                if (layerObject.meta.legend_type !== 'none' && layersDefinitions[i].show_legend) {
                  returnLegends.push(new window.JeoLegend(layerObject.meta.legend_type, {
                    layer_id: layerObject.slug,
            ...
            _this4.layers = returnLayers;
            _this4.legends = returnLegends;
    ...
    key: "getRelatedPosts",
    value: function getRelatedPosts() {
      var _this5 = this;

      return new Promise(function (resolve, reject) {
        var relatedPostsCriteria = _this5.getArg('related_posts');

        _this5.relatedPostsCriteria = relatedPostsCriteria;
        var query = {};
        query.per_page = 100; // TODO handle limit of posts per query

        query.orderby = 'date';
        query.order = 'desc';
        var keys = Object.keys(relatedPostsCriteria);
        ...
        query._embed = 1;
        jQuery.get(jeoMapVars.jsonUrl + 'posts', query, function (data) {
          if (data.length) {
            data.forEach(function (post) {
              _this5.addPostToMap(post);
            });
          }
        });
      });
    ...
    key: "addPostToMap",
    value: function addPostToMap(post) {
      var _this6 = this;

      if (post.meta._related_point) {
        post.meta._related_point.forEach(function (point) {
          _this6.addPointToMap(point, post);
        });
      }
    }
  }, {
    key: "addPointToMap",
    value: function addPointToMap(point, post) {
      var _this7 = this;

      var color = point.relevance === 'secondary' ? '#CCCCCC' : '#3FB1CE';
      ...
      marker.getElement().addEventListener('click', function () {
        _this7.activateMarker(marker);

        if (!_this7.options || !_this7.options.marker_action === 'embed_preview') {
          marker.setPopup(popUp);
        } else {
          _this7.embedPreviewActive = true;

          _this7.updateEmbedPreview(post);
        }
    ...
    key: "activateMarker",
    value: function activateMarker(activeMarker) {
      this.markers.map(function (marker) {
        var canToggle = marker._lngLat.lat === activeMarker._lngLat.lat && marker._lngLat.lon === activeMarker._lngLat.lon;
    ...
    key: "updateEmbedPreview",
    value: function updateEmbedPreview(post) {
      var html = this.popupTemplate({
        post: post,
        read_more: window.jeoMapVars.string_read_more,
    ...
    key: "getSwitchableLayers",
    value: function getSwitchableLayers() {
      var layers = [];
      this.layersDefinitions.forEach(function (el, index) {
    ...
    key: "getSwappableLayers",
    value: function getSwappableLayers() {
      var layers = [];
      this.layersDefinitions.forEach(function (el, index) {
        if (el.use === 'swappable') {
          layers.push(index);
    ...
    key: "getDefaultSwappableLayer",
    value: function getDefaultSwappableLayer() {
      var layers = [];
      this.layersDefinitions.forEach(function (el, index) {
    ...
    */
    ...
    key: "addLayersControl",
    value: function addLayersControl(amountLayers) {
      var _this8 = this;

      var switchableLayers = this.getSwitchableLayers();
      var swappableLayers = this.getSwappableLayers();
      var navElement = document.createElement('nav');
      navElement.classList.add('layers-selection');

      if (switchableLayers.length + swappableLayers.length !== 0 && amountLayers > 1) {
        var layerSelectionTitle = document.createElement('div');
        layerSelectionTitle.classList.add('layer-selection-title');
        var legendsTitle = document.createElement('div');
        legendsTitle.classList.add('legends-title');
        var legendTextIcon = document.createElement('div');
      ...
      switchableLayers.forEach(function (index) {
        if (_this8.layers[index]) {
          var link = document.createElement('a');
          link.href = '#';

          if (_this8.layersDefinitions[index].default) {
            link.className = 'active';
          }

          var layerName = document.createElement('span');
          layerName.classList.add('layer-name');
          layerName.textContent = decodeHtmlEntity(_this8.layers[index].layer_name);
          link.setAttribute('data-layer_id', _this8.layers[index].layer_id);

          link.onclick = function (e) {
            var clicked = e.currentTarget;
            var clickedLayer = clicked.dataset.layer_id;
            e.preventDefault();
            e.stopPropagation();

            var visibility = _this8.map.getLayoutProperty(clickedLayer, 'visibility');

            if (typeof visibility === 'undefined' || visibility === 'visible') {
              _this8.hideLayer(clickedLayer);

              clicked.className = '';
            } else {
              clicked.className = 'active';

              _this8.showLayer(clickedLayer);
            }
          };
      ...
      swappableLayers.forEach(function (index) {
        if (_this8.layers[index]) {
          var link = document.createElement('a');
          link.href = '#';
          link.classList.add('swappable');

          if (_this8.getDefaultSwappableLayer() == index) {
            link.classList.add('active');
          }

          link.textContent = decodeHtmlEntity(_this8.layers[index].layer_name);
          link.setAttribute('data-layer_id', _this8.layers[index].layer_id);
          ...
          link.onclick = function (e) {
            ...
            e.preventDefault();
            e.stopPropagation(); // hide all

            _this8.getSwappableLayers().forEach(function (i) {
              _this8.hideLayer(_this8.layers[i].layer_id);
            });

            jQuery(layers).children('.swappable').removeClass('active'); // display current
    ...
    key: "changeLayerVisibitly",
    value: function changeLayerVisibitly(layer_id, visibility) {
      var _this9 = this;

      this.map.getStyle().layers.forEach(function (layer) {
        if (layer.id == layer_id) {
          _this9.map.setLayoutProperty(layer.id, 'visibility', visibility);
        }
      });
    }
  }, {
    key: "showLayer",
    value: function showLayer(layer_id) {
      this.changeLayerVisibitly(layer_id, 'visible');
      jQuery(this.element).find('.legend-for-' + layer_id).show();
    }
  }, {
    key: "hideLayer",
    value: function hideLayer(layer_id) {
...
/******/ });
//# sourceMappingURL=jeoMap.js.map