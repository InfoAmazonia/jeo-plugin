...
class Minimap_Output {
	 ...
	 */
	#[SchemaProperty(
		description: 'Thematic map layer definitions, ordered bottom-to-top: raster layers first, then vector layers (opaque rasters must never hide vector data). Each entry has: id (int), use ("fixed"), default (true), show_legend (bool), reason (optional string — briefly explain why this layer was selected), theme (optional string — the primary layer-theme taxonomy term), style (optional object). The style object can contain: use_default (bool — when true, use the layer\'s AI-suggested default_style), filter (array — MapLibre filter expression like ["==", "class", "wood"]), paint (object — paint properties like {"fill-color": "#2d5a27", "fill-opacity": 0.6}), layout (object — layout properties).',
		required: true,
	)]
