...
class Fixtures {
		...
		$posts = array(
			...
			array(
				...
				'meta'        => array(
					...
					'max_bounds_sw'       => array( '-73.248291', '-43.867948' ),
					'max_bounds_ne'       => array( '33.714600', '61.958488' ),
	...
	private function get_posts() {
		...
		foreach ( $schema as $item ) {
			...
			if ( $post ) {
				$results[] = array_merge( array( 'ID' => $post->ID ), $item );
			}
		}
	...
	public function update() {
		...
		if ( ! $term1 ) {
			$term1 = wp_insert_term( 'category 1', 'category' );
		}
		if ( ! $term2 ) {
			$term2 = wp_insert_term( 'category 2', 'category' );
		}
		if ( ! $term3 ) {
			$term3 = wp_insert_term( 'category 3', 'category' );
		}
		...
		$wpdb->update( $wpdb->posts, array( 'post_content' => $this->get_sample_onetime_map_div() ), array( 'ID' => $sample_1_id ) );
		...
		$wpdb->update( $wpdb->posts, array( 'post_content' => $this->get_sample_map_div() ), array( 'ID' => $sample_2_id ) );
