...
- **AI Context Assistant**: Editorial suggestion sidebar for posts. Uses the same Assistant architecture as the minimap but with distinct agent, tools, and output DTO. Key patterns and regression guards:
  ...
  - **Retry logic**: `run_agent()` retries up to 3 times with exponential backoff (`min(attempt * 2, 8)` seconds) on empty responses, network errors, 5xx, and 429 responses. After exhausting retries, it throws a user-friendly exception.
  ...
  - **Chat message storage (dual storage)**: The UI chat history is stored separately from the AI context history to avoid JSON schema pollution from structured output:
    ...
    - `api_clear()` deletes four meta keys (`_jeo_ai_context_conversation_id`, `_jeo_ai_context_last_response`, `_jeo_ai_context_chat_messages`, `_jeo_ai_context_suggestion_history`), resetting the conversation and suggestion history completely.
  - **Suggestion history**: The panel keeps a versioned history of generated suggestion sets (`_jeo_ai_context_suggestion_history`), allowing users to browse and restore previous outputs. History UI lives in `context-chat-panel.js`.
