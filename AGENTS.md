...
- **Minimap render order**: `Minimap::normalize_layer_render_order()` stable-sorts layers bottom-to-top as raster → vector (base layer is separate and always below). Raster group: `tilelayer`, `mapbox-tileset-raster`, plus `mapbox` styles whose style JSON contains raster sub-layers (Mapbox Styles API check, transient-cached; fetch failure = raster, conservatively). Vector group: `mvt`, `mapbox-tileset-vector`, vector-only `mapbox` styles. All render paths stack by array order, so this covers editor, frontend, and composed styles. Applied in `run_agent()` (all AI paths) and legacy `api_setup()` (RAG).
