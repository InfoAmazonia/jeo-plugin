...
- **AI Context Assistant**: Editorial suggestion sidebar for posts. Uses the same Assistant architecture as the minimap but with distinct agent, tools, and output DTO. Key patterns and regression guards:
  ...
  - **REST handler**: `Context_Handler` (`class-context-handler.php`), Singleton, registers `/jeo/v1/context/setup` and `/jeo/v1/context/chat`. Uses the same `inject_history` / `persist_history` pattern as `Minimap`. Changing the history format will break existing conversations stored in post meta.
  ...
  - **State persistence**: Conversation ID, messages, and last suggestions are persisted in post meta (`_jeo_ai_context_conversation_id` and `_jeo_ai_context_last_response`). On mount, the frontend calls `GET /jeo/v1/context/state` to restore the previous session. This allows closing/reopening the panel or refreshing the page without losing context.
  - **Editor integration**: Paragraphs are inserted via `createBlock('core/paragraph', { content: text })` + `wp.data.dispatch('core/block-editor').insertBlocks()`. There is also a "Copy" button that uses `navigator.clipboard.writeText()`. Do not change the block type without updating the import from `@wordpress/blocks`.
