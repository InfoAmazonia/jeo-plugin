...
- **Iframe compat**: Extensive patches for Gutenberg Block API v3 (iframe editor) — `instanceof` duck-typing, event listener rebinding, fullscreen control patches, missing-CSS detection, ResizeObserver. The layer-type registry is loaded inside the iframe via `Layer_Types::enqueue_iframe_assets()` (`enqueue_block_assets`, admin-only) so editor bundles can use `window.JeoLayerTypes` metadata (`isStyle`, schemas, `getStyleUrl`) — `map-blocks/use-style-layer.js` detects style types through `shared/style-layer-types.js::isStyleLayerType()` (registry-driven, no hardcoded slugs).
- **Editor↔Frontend parity**: The editor preview and frontend use separate rendering code but must match. For `load_as_style` (style-type layers — `mapbox` or `style-json` — loaded as the map's base style), use `map-blocks/use-style-layer.js` (`findStyleLayer`, `applyStyleLayerFiltering`) — mirrors `class-jeo-map.js::applyBaseStyleLayer()` frontend logic. `map-preview-layer.js::renderLayer` returns `null` for style layers. All map editors (minimap, map, onetime-map, storymap) are integrated.
...
- **Attribution display**: The `attribution` meta holds either a plain URL or trusted small HTML (e.g. base layers: `&copy; <a href="...">OpenFreeMap</a>`). Editor panels render it via the shared `shared/html.js::sanitizeHtml()` (allowlist `a[href]`/`br`/`strong`/`em`/`i`/`span`, forced `target="_blank" rel="noopener noreferrer"`); the frontend legend template (`templates/map-content-layers-list.php`) renders HTML attributions with `wp_kses_post()` and keeps the `<a href>` URL wrap for plain values. Never wrap an HTML attribution in another `<a>` — nested anchors are invalid HTML (the map attribution control passes HTML attributions through as-is).
...
- **Mapbox style fetch helper**: `Jeo::fetch_mapbox_style( $style_id, $token, $args )` (class-jeo.php) is the single Mapbox Styles API client, used by the composer and the minimap. Successful fetches are transient-cached (`jeo_mapbox_style_json_*`, TTL filter `jeo_mapbox_style_cache_ttl`, default 1h; `bypass_cache` arg to skip). The cache is purged by the composer on `save_post_map-layer` and on forced refresh (`refresh` REST param / layer cache-refresh endpoint) via `Jeo::delete_mapbox_style_cache()`. Never fetch Mapbox styles inline — reuse this helper. Its generic keyless counterpart `Jeo::fetch_style_json( $style_url, $args )` (same caching pattern, `jeo_style_json_*` / `jeo_style_json_cache_ttl`, purge via `Jeo::delete_style_json_cache()`) serves the `style-json` layer type and any other keyless style provider.
- **Style layer types (`style-json`)**: `mapbox` and `style-json` are style types — they load a whole MapLibre GL style that becomes the map's base style, never individual GL layers. The contract is declared per type: JS `isStyle: true` on `JeoLayerTypes.registerLayerType()` (+ optional `getStyleUrl`/`getInlineStyle`/`getStyle`), PHP `is_style => true` on `Layer_Types::register_layer_type()` (`Layer_Types::is_style()`, filterable via `jeo_layer_type_is_style`). Consumers must key off the flag, never the type slug. The `style-json` schema mirrors `geojson`: `style_url` (public style JSON URL) or `inline_style` (raw JSON text, takes precedence), no API key required. The composer merges any `is_style` type with `style_url`/`inline_style` as a bottom bundle (fetch via `fetch_style_json()`); on keyless sites the frontend applies the base via `map.setStyle()` (`class-jeo-map.js::applyBaseStyleLayer`, mirrored in `storymap-display.js`). A future provider needs only the two registrations.
- **Minimap base layers**: defaults are runtime-aware (`get_default_base_layers()`): sites with a Mapbox key get `mapbox` styles, keyless sites get `style-json` (OpenFreeMap Dark) for `dark`, OSM tiles for `light`, and Esri for `satellite` — the old CARTO dark tilelayer no longer works (API key now required). Base CPTs matching a known legacy signature (CARTO dark raster) are rewritten in place with the current default by `repair_base_layer_if_legacy()` — lazily on every base lookup and eagerly via `maybe_migrate_base_layers()` on `admin_init` (versioned by `Minimap::BASE_LAYERS_MIGRATION_VERSION`; bump when defaults change). User-customized bases are never touched. Filterable via `jeo_minimap_base_layers`.
- **Boundary layer generation**: `Place_Polygon_Service` (`class-place-polygon-service.php`) resolves a place name into an authoritative polygon using adapter-specific public sources: IBGE for Brazilian municipalities/states (honors the `entity_type` hint and treats a non-Brazilian context as "not applicable" — e.g. "França" with `entity_type=state` must not accent-match Franca/SP), FUNAI WFS for indigenous lands (endpoint `geoserver.funai.gov.br`, feature `Funai:tis_poligonais`, name attribute `terrai_nome`, OGC `PropertyIsLike` filter with `matchCase="false"` — the old `catalogo.funai.gov.br` CQL/`strToLower` endpoint is dead), and Nominatim (`polygon_geojson=1`, geometry decoded as stdClass for memory) with Overpass (POST, mirrors overpass-api.de → overpass.osm.ch → maps.mail.ru, 25s/mirror) as fallback for international boundaries. Results are cached per place, deduplicated by content hash, and published as WordPress attachment GeoJSON files for stable public URLs. Adapter `null` = "not applicable" (try next); adapter `WP_Error` = that source failed, but the remaining adapters still run (last error returned only if all fail). Boundary layers are created with type `geojson` (`layer_type_options.data` = attachment URL; the boundary fill colors are stored in the `default_style` post meta, not in `layer_type_options`) and rendered **client-side** by the `geojson` layer type (`layer-types/geojson.js`): a single `fill` GL layer whose outline comes from the `fill-outline-color` paint prop (~1px antialiasing halo, no configurable width/opacity — the old `{layer_id}__outline` companion line layer was replaced; the legacy toggle in `class-jeo-map.js::changeLayerVisibitly` remains only for layers created before the switch). No Mapbox style publishing is involved: the Mapbox Styles API rejects `geojson` sources in hosted styles (HTTP 422). The `geojson` type is also supported by the editor preview (`map-preview-layer.js`) and the composer (direct-type inlining, one manifest `compositeLayers` entry per GL layer). The type accepts either `data` (public GeoJSON URL) or `inline_geojson` (raw JSON text, takes precedence), and its render type is keyed by `layer_type_options.type` — only `fill` is exposed in the schema for now, with the renderers structured as switches ready for future types (line, circle, …). Styling lives **on the map layer instance** (`layers[i].style`, the same flat `{ use_default, paint, layout }` shape used by `mvt`/`mapbox-tileset-vector`), never on the layer CPT: when the instance sets `use_default`, the layer's `default_style` meta takes over. All renderers resolve the effective style identically — editor preview, frontend runtime (`class-jeo-map.js` forwards `style`/`default_style` to `JeoLayer`), and composer (`resolve_effective_layer_style()` in `class-map-style-composer.php`). Paint defaults are shared with the rest of the plugin via `JeoLayerTypes.getFallbackPaint('fill')` (`layer-types/JeoLayerTypes.js`, mirrored in the composer's PHP default paint — keep both in sync), and instance opacity multiplies numeric opacity props of the merged paint. It uses only style-spec v8 core APIs, so it is compatible with both the MapLibre GL JS and Mapbox GL JS runtimes. Tool failures are logged to the PHP error log with the `[JEO]` prefix.
- **Mapbox style builder**: `Mapbox_Style_Builder` (`class-mapbox-style-builder.php`) constructs and publishes Mapbox GL styles deterministically to the Mapbox Styles API for minilayer composed styles, replacing the Mapbox DevKit MCP. It extracts the Mapbox username from the access token's JWT-like payload and validates published styles via `Jeo::fetch_mapbox_style()`. Publishing requires the `styles:write` scope — the usual public token (`pk.…`) fails with HTTP 403, which `publish_style()` translates into an actionable message; the publish token is resolved by `jeo_settings()->get_mapbox_publish_token()` (server-only `mapbox_secret_key` wins, public `mapbox_key` is the fallback).
- **Boundary tool**: `Generate_Boundary_Layer_Tool` is registered in `Tool_Registry` and exposed to the minimap agent **unconditionally** (no Mapbox key required — boundary layers render client-side). It may be called proactively for administrative boundaries and indigenous lands; non-boundary layers still require explicit user authorization via `Generate_Layer_Tool`.
...
- **AI feature gating**: Every AI-dependent feature (UI or endpoint) must be gated by `AI_Handler::is_configured()` (valid provider slug + its key, or `ollama_url` for Ollama; filter `jeo_ai_is_configured`) — never `empty( ai_default_provider )` (it defaults to `'gemini'`). The flag is exposed to JS as `ai_configured` in both the `jeo` (parent document, posts-sidebar) and `jeoMapVars` (block editor bundles) localizes. When not configured: AI-only controls are hidden completely (no CTA), the `jeo-context-sidebar` asset is not enqueued, and minimap REST endpoints return a friendly `400`. Generated content stays fully editable — only AI chat/generation is removed.
- **Minilayer**: Thematic layer generation uses `Minilayer_Classifier` (single structured-output call) + `Minilayer_Service` + `Mapbox_Style_Builder`. The Mapbox DevKit MCP path and `JEO_AI_Factory::create_minilayer_assistant()` have been removed. `Generate_Boundary_Layer_Tool` handles boundaries deterministically; `Generate_Layer_Tool` handles thematic layers. Generated `map-layer` CPTs are enriched by `Minilayer_Metadata` with a `post_excerpt` description (classifier `description` field or a deterministic boundary template — never disclaimers like "Generated by AI" or "simplified geometry"), `layer-theme` taxonomy, attribution, and an auto-generated `simple-color` legend when a representative color exists. Attribution is a short source credit only (e.g. `IBGE`, `OpenStreetMap contributors`, `Mapbox tileset %s`) — the UI already labels the field ("Source:"/"Fonte:"), so the stored text never carries its own prefix; `limitations` stays agent-facing and is never copied into attribution.
- **Minimap**: AI-assisted map block (`jeo/ai-minimap`). Follows the same Assistant architecture as Minilayer/Context Assistant with structured output (`Minimap_Output`), conversation storage (`WP_Storage`), and optional Minilayer integration. See `.architecture/minimap/README.md` before modifying endpoints, tools, or editor state. Pin preview parity: when `show_pins` is enabled, the editor preview renders `pins` as `<Marker>` children of `<Map>` (re-exported from `lib/mapgl-react.js`), mirroring the frontend's `addOwnPinsAsMarkers()` — icons come from `jeoMapVars.images`, shared by both localizations via `Jeo::get_map_icon_images()` (filter `jeomap_js_images`); markers resolve the active GL runtime (MapLibre/Mapbox) from the `<Map>` context, so they work under both runtimes.
...
- **AI Context Assistant**: Editorial suggestion sidebar for posts. Uses the same Assistant architecture as the minimap but with distinct agent, tools, and output DTO. Key patterns and regression guards:
  ...
  - **Critical prompt rules**: `Context_Agent::critical_prompt_rules()` contains non-negotiable instructions for inline contextual links, factual grounding, recency tie-breaking (when retrieved articles have similar relevance scores, the more recently published one is preferred — tie-breaker only, enabled by the `date` field exposed by `retrieve_knowledge`), and preferring internal site links over external sources. The default prompt concatenates these rules automatically. Custom prompts should preserve them; use the **Prompt Engineering Assistant** to enforce them.
  - **Multi-theme retrieval**: requests combining N themes (N ≥ 2) must run one `retrieve_knowledge` query per theme — via the tool's `queries` array (1–6 strings), merged/deduped by `post_id` and interleaved by per-query rank so every theme keeps representation under the `top_k` cap. Never blend themes into a single query (dense retrieval dilutes coverage). Generation prefers one integrated paragraph when sources support it, falling back to one paragraph per theme with a plain note in `assistant_message`.
  ...
  - **Error persistence**: On retry exhaustion in `api_chat()`, the user message is persisted to **both** stores (UI messages saved before `run_agent()`; ConversationStore appended after retries exhausted). This ensures the message survives page reload and the AI retains context for subsequent turns. The ConversationStore append is always the pair `user` + a synthetic `assistant` failure note so the stored thread keeps strict user/assistant alternation (the AI library validates alternation on every replayed message and rejects same-role adjacency with "Invalid message sequence"). `api_setup()` does not persist on error. The Minimap follows the same pattern: frontend preserves the user message in block attributes on `.catch()`; backend appends the pair to ConversationStore in `catch` blocks of `api_chat()` and `api_setup_prompt()`. Both handlers replay threads through `Jeo\AI\Thread_Normalizer::normalize_thread_messages()` (merge consecutive same-role messages, drop leading assistant / trailing orphan user), which also self-heals threads poisoned before the fix.
