...
- **Minilayer**: `Mapbox_Style_Builder` is replacing the Mapbox DevKit MCP path. `Generate_Boundary_Layer_Tool` already bypasses the MCP for boundary layers. The remaining `Generate_Layer_Tool` / `Minilayer_Service` thematic path will be migrated to the deterministic builder in the next phase. Until then, `JEO_AI_Factory::create_minilayer_assistant()` still loads the MCP config for non-boundary layers.
