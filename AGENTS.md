...
- **Async mode**: All editor blocks (`jeo/map`, `jeo/onetime-map`, `jeo/layer-editor`, `jeo/map-editor`, etc.) wrap their `edit` component in `<AsyncModeProvider value={ true }>` to batch re-renders from `wp.data` store changes.
