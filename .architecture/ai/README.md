...
| `src/includes/ai/class-context-handler.php` | REST handler: `/context/setup` and `/context/chat`, conversation history inject/persist |
| `src/includes/ai/class-context-generation-output.php` | Structured output DTO: paragraphs, references, message, assistant_message |
...
| `src/js/src/context-sidebar/suggested-paragraphs.js` | Renders suggested paragraphs with "Insert into article" button (creates `core/paragraph` blocks) |

### Features
- **Manual setup**: User clicks "Generate Suggestions" to start; no automatic trigger on panel open.
- **State persistence**: Conversation ID, messages, and last suggestions are stored in post meta (`_jeo_ai_context_conversation_id`, `_jeo_ai_context_last_response`). Closing/reopening the panel or refreshing the page restores the session via `GET /context/state`.
...
- **Copy / Insert**: Each suggested paragraph has both "Copy to clipboard" and "Insert into article" buttons.
