...
See [`context/README.md`](../context/README.md) for full architecture, agent details, REST endpoints, and data flows.

The Context Assistant is a Gutenberg sidebar plugin that suggests new paragraphs and related article references. Key features:

- **Manual setup**: User clicks "Generate Suggestions" to start; no automatic trigger.
- **Dual conversation storage**: `ConversationStore` for AI context, separate `post_meta` for clean UI messages.
- **Structured output**: `Context_Generation_Output` DTO with paragraphs (inline HTML), references, messages.
