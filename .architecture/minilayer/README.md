...
| `src/includes/ai/class-minilayer-metadata.php` | `Minilayer_Metadata` | Helpers for post description, `layer-theme` taxonomy, and `simple-color` legend generation from layer paint |
...
sequenceDiagram
    ...
    S->>MD: Minilayer_Metadata (description, theme, legend)
...
|--------------|--------|---------|
| `post_excerpt` | `Minilayer_Metadata::build_description()` — classifier `description` field or deterministic boundary template | Description paragraph shown in layer library / minimap disambiguation; no disclaimers (no "Generated by AI", "simplified geometry", etc.) |
| `attribution` | Deterministic source credit (e.g. `IBGE`, `OpenStreetMap contributors`, `Mapbox tileset %s`) | Credit only — the UI already labels the field ("Source:"/"Fonte:"), so the stored text must not carry its own prefix; limitations are agent-facing only and never copied into attribution |
...
5. **Theme and Metadata** — `layer_title`, `description`, `theme`, and `limitations` guidance
