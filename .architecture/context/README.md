...
| `src/includes/ai/class-retrieve-knowledge-tool.php` | NeuronAI tool — queries `jeo_knowledge` via `RAG_Agent::resolveRetrieval()`. Returns `post_id`, `title`, `excerpt`, `url`, `date` (ISO `Y-m-d`, from `WP_Post_Data_Loader` metadata), and `score` per result |
